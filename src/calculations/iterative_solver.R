...
solve_starting_income_iterative <- function(df, 
  ...
  # Prepare parameters OUTSIDE the loop (they don't change)
  ...
  # This is needed for state tax calculations that reference final_federal_income_tax
  df <- df %>%
    mutate(
  ...
  # Debug: Print columns being returned
  cat("\n[DEBUG] Columns returned from iterative solver:\n")
