...
print_convergence_summary <- function(df, debug = FALSE) {
  ...
  cat("\n=== Mean Tax Components ===\n")
  cat(sprintf("Federal payroll: $%.2f\n", mean(dplyr::coalesce(df$total_fed_payroll_tax, 0), na.rm = TRUE)))
  cat(sprintf("State payroll:   $%.2f\n", mean(dplyr::coalesce(df$state_payroll_tax, 0), na.rm = TRUE)))
  cat(sprintf("Local income:    $%.2f\n", mean(dplyr::coalesce(df$local_income_tax, 0), na.rm = TRUE)))
  cat(sprintf("Federal net:     $%.2f\n", mean(dplyr::coalesce(df$federal_tax_liability_with_refund, 0), na.rm = TRUE)))
